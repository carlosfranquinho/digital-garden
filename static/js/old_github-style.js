          ...
          <details class="Details-element details-reset" open>
            <summary role="button" class="btn-link f4 muted-link no-underline lh-condensed width-full">
              <span class="color-text-primary ws-normal text-left">
                Escrevi ${monthPosts.length} nota${
...
function graph(year, posts, startDate, endDate) {
  const postsStr = posts.length === 1 ? "nota" : "notas";
  if (year === now.getFullYear().toString()) {
    document.querySelector(
      "#posts-count"
    ).innerText = `${posts.length} ${postsStr} no último ano`;
  } else {
    document.querySelector(
      "#posts-count"
    ).innerText = `${posts.length} ${postsStr} em ${year}`;
  }
...
function svgTip(elem, count, dateStr) {
  ...
  const dateFmt = `${date.getDate()} ${
    months[date.getMonth()]
  } ${date.getFullYear()}`;
  if (count) {
    svgElem.innerHTML = `<strong>${count} notas</strong> em ${dateFmt}`;
  } else {
    svgElem.innerHTML = `<strong>Sem notas</strong> em ${dateFmt}`;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  const imagens = document.querySelectorAll(".taped-image");

  imagens.forEach((el) => {
    const rotacaoImagem = (Math.random() * 4 - 2).toFixed(2); // entre -2° e +2°
    const rotacaoFitaTopo = (Math.random() * 10 - 5).toFixed(2); // entre -5° e +5°
    const rotacaoFitaFundo = (Math.random() * 10 - 5).toFixed(2);
    const posicaoFitaTopo = (Math.random() * 20 + 5).toFixed(2); // entre 5% e 25%
    const posicaoFitaFundo = (Math.random() * 20 + 5).toFixed(2);

    el.style.setProperty("--rotacao-img", `${rotacaoImagem}deg`);
    el.style.setProperty("--rotacao-topo", `${rotacaoFitaTopo}deg`);
    el.style.setProperty("--rotacao-fundo", `${rotacaoFitaFundo}deg`);
    el.style.setProperty("--pos-topo", `${posicaoFitaTopo}%`);
    el.style.setProperty("--pos-fundo", `${posicaoFitaFundo}%`);

    // Gerar perturbações de 0 a ±2%
    const randomOffset = (base, range = 2) =>
    ...
    points.forEach(([varName, base]) => {
      el.style.setProperty(varName, randomOffset(base));
    });
  });
});
